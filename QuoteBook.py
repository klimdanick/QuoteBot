...
class Stats:
    def __init__(self, quotes):
        self.quotes = quotes
        self.leaderboard = self.__calc_leaderboard()
        self.totalAuthors = len(self.leaderboard.keys())
...
class QuoteBook:
            ...
            json.dump(
                {
                    "quotes": [
                        {
                            ...
                            "date": (
                                quote.date_added
                                if quote.date_added is not None
                                else None
                            ),
                        }
                        for quote in self.quotes
                    ...
                    "metaData": (
                        {"time": self.metaData.time, "currentQuote": current_quote_dict}
                        if self.metaData
                        else None
                    ),
                },
    ...
    def get_stats(self, key):
        stats = Stats(self.quotes)
        return stats.from_key(key)
